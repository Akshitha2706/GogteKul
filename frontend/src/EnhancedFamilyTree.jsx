...
const EnhancedFamilyTree = () => {
  ...
  useEffect(() => {
    const fetchTreeData = async () => {
      try {
        ...
        const normalizedRelationships = (relationshipsRes.data || []).map(rel => ({
          ...rel,
          fromSerNo: rel.fromSerNo != null ? Number(rel.fromSerNo) : null,
          toSerNo: rel.toSerNo != null ? Number(rel.toSerNo) : null,
        ...
        members.forEach(member => {
          const normalizedMember = {
            ...member,
            serNo: member.serNo != null ? Number(member.serNo) : null,
            level: member.level != null ? Number(member.level) : member.level,
            fatherSerNo: member.fatherSerNo != null ? Number(member.fatherSerNo) : null,
            motherSerNo: member.motherSerNo != null ? Number(member.motherSerNo) : null,
            spouseSerNo: member.spouseSerNo != null ? Number(member.spouseSerNo) : null,
            childrenSerNos: Array.isArray(member.childrenSerNos)
              ? member.childrenSerNos.map(child => Number(child)).filter(child => !Number.isNaN(child))
              : [],
            fullName: member.fullName || `${member.firstName || ''} ${member.middleName || ''} ${member.lastName || ''}`.trim(),
            dob: member.dob || member.dateOfBirth || member.birthDate || null
          };
          if (normalizedMember.serNo != null) {
            memberMap.set(normalizedMember.serNo, normalizedMember);
          }
        });

        const childRelationLabels = new Set(['son', 'daughter', 'child']);
        const visited = new Set();

        const buildTree = (currentSerNo) => {
          if (currentSerNo == null || visited.has(currentSerNo)) return null;
          const currentMember = memberMap.get(currentSerNo);
          if (!currentMember) return null;

          visited.add(currentSerNo);

          const directChildIds = new Set(
            currentMember.childrenSerNos || []
          );

          normalizedRelationships.forEach(rel => {
            if (rel.fromSerNo === currentSerNo) {
              const relationLabel = rel.relation?.toLowerCase?.() || '';
              if (childRelationLabels.has(relationLabel) && rel.toSerNo != null) {
                directChildIds.add(rel.toSerNo);
              }
            }
          });

          const children = Array.from(directChildIds)
            .map(childSerNo => buildTree(childSerNo))
            .filter(Boolean);
        ...
        const resolvedRootSerNo = treeRes.data?.serNo != null
          ? Number(treeRes.data.serNo)
          : (serNo ? Number(serNo) : members[0]?.serNo);

        const computedTree = buildTree(resolvedRootSerNo);

        if (computedTree) {
          setTreeData(computedTree);
          setExpandedNodes(new Set([String(computedTree.serNo)]));
        } else {
          const fallbackRoot = treeRes.data || null;
          if (fallbackRoot) {
            setTreeData(fallbackRoot);
            const fallbackSerNo = fallbackRoot.serNo != null ? fallbackRoot.serNo : serNo || 1;
            setExpandedNodes(new Set([String(fallbackSerNo)]));
          } else {
            setTreeData(null);
  ...
  const getRelationshipInfo = (fromSerNo, toSerNo) => {
    if (fromSerNo == null || toSerNo == null) return null;
    const from = Number(fromSerNo);
    const to = Number(toSerNo);
    const relationship = relationships.find(
      rel => rel.fromSerNo === from && rel.toSerNo === to
    );
    return relationship ? {
  ...
  const MemberNode = ({ member, level = 0, parentSerNo = null }) => {
    const nodeId = String(member.serNo);
    ...
    return (
      <div className="relative">
        ...
        <div className={`bg-white rounded-lg shadow-md border-2 mb-4 transition-all duration-200 ${
          Number(member.serNo) === Number(serNo || treeData?.serNo) 
